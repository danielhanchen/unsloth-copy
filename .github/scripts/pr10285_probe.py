...
def network_allowlist() -> int:
    ...
    if sys.platform == "win32":
        ...
        fetch = "python3 -c \"import urllib.request; print('PR10285_NET pypi=' + str(urllib.request.urlopen('https://pypi.org/simple/pip/', timeout=30).status))\" 2>&1 | tail -1 | sed 's/^/PR10285_NET fetch=/'; python3 -c \"import urllib.request; print('PR10285_NET hf=' + str(urllib.request.urlopen('https://huggingface.co/api/models/gpt2', timeout=30).status))\" 2>&1 | tail -1 | sed 's/^/PR10285_NET hffetch=/'; git ls-remote --exit-code https://github.com/pypa/pip.git HEAD 2>&1 | tail -1 | cut -c1-60 | sed 's/^/PR10285_NET git=/'; python3 -c \"import ssl,os; p=ssl.get_default_verify_paths(); print('PR10285_NET ssl=', p.cafile, os.path.exists(p.cafile or ''), p.capath, os.path.isdir(p.capath or ''))\""
