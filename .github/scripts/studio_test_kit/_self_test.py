...
def test_compose_images() -> None:
    ...
    try:
        ...
        # Unequal heights must be PADDED, not scaled: a before/after pair whose whole
        # point is "the panel got taller" is worthless if the shorter half is silently
        # enlarged to match. Widths staying native is what proves no resize happened.
        tall = tmp / "tall.png"
        Image.new("RGB", (200, 400), "blue").save(tall)
        padded = hstack_images(left, tall, tmp / "pad.png")
        with Image.open(padded) as im:
            assert im.width == 320 + 24 + 200, im.width
            assert im.height == 400 + 56, im.height
        scaled = hstack_images(left, tall, tmp / "scaled.png", scale_to_match=True)
        with Image.open(scaled) as im:
            assert im.width > 320 + 24 + 200, im.width  # the short half was stretched
