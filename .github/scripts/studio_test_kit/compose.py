...
def hstack_images(
    ...
    scale_to_match: bool = False,
) -> Path:
    """Stack two screenshots horizontally with column labels.

    Unequal heights are PADDED onto a common canvas, not scaled. When a PR makes a
    panel taller -- which is most of what a before/after pair is for -- scaling the
    shorter half blows its text up to a different size and the pair reads as
    retouched rather than as two photographs of the same UI. Padding keeps both
    halves at 1:1, so any difference in the picture is a difference in the product.

    `scale_to_match=True` restores the old resize, for pairs shot at genuinely
    different viewport sizes where the height difference is not the finding.
    """
    if Image is None:
