...
def isolated(monkeypatch, tmp_path):
    ...
    connection.close()
    # Alice loaded the resident video model, so she holds the GPU; the model is public, which is
    # what lets Bob generate on it.
...
def test_the_starting_account_owns_its_clip_and_the_models_loader_does_not(backend):
    ...
    with _client(BOB) as client:
        ...
        # Bob polls his own job.
        assert client.get(progress).json()["video"]["prompt"] == CLIP["prompt"]

    with _client(ALICE) as client:
        # Alice loaded the model, but the clip, its prompt and its cancel are Bob's.
