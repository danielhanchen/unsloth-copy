...
# A 1x1 PNG, so the labeling grid endpoints see a decodable image rather than arbitrary bytes.
PNG_1X1 = (
    "iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAADUlEQVR42mP8z8BQDwAEhQGAhKmM"
    "IQAAAABJRU5ErkJggg=="
)

# The reasons below, kept next to the factories that cite them.
_SCAN_FOLDER_REASON = (
...
def seed_training_start_request(account) -> dict[str, str]:
    """A finished training start request owned by the account, in the backend's own table."""
    ...
    def install() -> None:
        ...
        # account_id defaults to the acting account, which is what these routes authorize against.
...
def seed_diffusion_run(account) -> dict[str, str]:
    """A persisted diffusion run record under the account's own runs directory."""
    import json
...
def seed_diffusion_dataset(account) -> dict[str, str]:
    """A dataset folder holding one captioned image under the account's datasets root."""
    import base64
...
def seed_scan_folder(account) -> dict[str, str]:
    """A registered custom model scan folder row in the account's own studio.db."""
...
def seed_recipe_job(account) -> dict[str, str]:
    """A completed data recipe job owned by the account, in the manager's in-process registry."""
    ...
    def install() -> None:
        ...
        # Preview, not full: publishing a full run uploads to the Hub, which this matrix cannot do.
...
def seed_unstructured_upload(account) -> dict[str, str]:
    """An unstructured seed block with one extracted file under the account's uploads root."""
    import json
...
# Both verbs share one handler, and the foreign 404 is immediate; the owner's stream is the problem.
SKIPPED = {
