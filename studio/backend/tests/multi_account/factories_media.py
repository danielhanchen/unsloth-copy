...
"""Media domain: the image, audio and video galleries, search thumbnails, chat tool sandboxes,
the API monitor and the OpenAI model retrieval route."""
...
# The store validates search ids as exactly twelve lowercase hex characters.
SEARCH_IMAGE_ID = "a1b2c3d4e5f6"
# The catalog names a standalone GGUF by its file stem, quant label included.
MODEL_ID = "media-local-model-Q4_K_M"

# Filled per seed with the link the product mints, since these two routes are token-gated.
...
def seed_image(account) -> dict[str, str]:
    """Persist one gallery PNG through the product's own writer, and mint its signed link."""
...
def seed_audio(account) -> dict[str, str]:
    """Persist one gallery WAV plus its recipe sidecar through the product's own writer."""
...
def _mp4_bytes() -> bytes:
    """A real four-frame H.264 clip, so the export route has something to transcode."""
    import io
...
def seed_video(account) -> dict[str, str]:
    """Persist one gallery MP4 plus its recipe sidecar, and mint its signed link."""
...
def seed_search_image(account) -> dict[str, str]:
    """Drop a cached search thumbnail where the product's own cache resolver reads it."""
    import io
    ...
    # No public writer: the fetch path is what normally fills this directory, and it needs network.
...
def seed_sandbox(account) -> dict[str, str]:
    """Create this chat's tool sandbox through the product's resolver and leave a file in it."""
    import os
...
def seed_monitor(account) -> dict[str, str]:
    """Record one API monitor row for this account's own subject."""
...
def seed_model(account) -> dict[str, str]:
    """Put a standalone GGUF in a scan folder of this account and let it into the catalog."""
    import struct
