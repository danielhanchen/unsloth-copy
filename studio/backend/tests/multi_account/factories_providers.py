...
"""Factories for the provider, ChatGPT/Codex OAuth, MCP refresh, prompt and account routes."""
...
# Filled by the migrate seeder: the body must carry a key encrypted with this process's RSA
# public key, which only exists once the product's key exchange has been initialized.
MIGRATION_BODY: dict = {"encrypted_api_key": ""}


def _encrypt_api_key(plaintext: str) -> str:
    """Encrypt with the product's own RSA public key, exactly as the browser does."""
    import base64
...
def _create_provider(provider_id: str, provider_type: str, display_name: str) -> None:
    """Idempotent, so a seeder stays safe to call twice inside one account."""
...
def seed_provider(account) -> dict[str, str]:
    """A saved OpenAI connection plus the encrypted API key that belongs to it."""
...
def seed_provider_without_key(account) -> dict[str, str]:
    """A saved connection with no key yet, which is the only state migrate may fill."""
    from utils.account_context import run_as
...
def seed_codex_provider(account) -> dict[str, str]:
    """A ChatGPT subscription connection that has never been authorized."""
    from utils.account_context import run_as
...
def seed_codex_connected(account) -> dict[str, str]:
    """A ChatGPT connection whose OAuth bundle is saved in this account's database."""
    from core.inference import openai_codex_auth as codex_auth
    from utils.account_context import run_as
...
def seed_codex_flow(account) -> dict[str, str]:
    """A pending authorization flow persisted in this account's credential table."""
    import time
...
def seed_mcp_refresh(account) -> dict[str, str]:
    """An MCP server row whose address refuses connections, so the probe returns at once."""
...
def seed_account(account) -> dict[str, str]:
    """The account row itself: these routes administer accounts, they do not belong to one."""
