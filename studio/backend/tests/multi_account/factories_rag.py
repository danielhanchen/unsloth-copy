...
"""Route isolation factories for routes.rag: knowledge bases, documents, jobs and linked folders."""
...
def _connection(account):
    """Open the acting account's rag.db through the product's own opener."""
...
def _store_file(account) -> str:
    """Write the source file the document row points at, inside the account's uploads root."""
...
def seed_rag_job(account) -> dict[str, str]:
    """A completed ingestion job with no live worker, so the SSE stream ends at once."""
    from core.rag import store
...
def seed_rag_folder(account) -> dict[str, str]:
    """A knowledge base with a linked folder and one finished sync job for it."""
...
# Reasons for every expectation that leaves the plain owner/other-account 404 contract.
_LIST_SCOPE = (
