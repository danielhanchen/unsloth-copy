...
"""Chat attachment, project delete, thread fork and durable chat generation run object routes."""
...
# The durable-run route only persists this envelope, so a minimal local request is enough.
RUN_REQUEST_PAYLOAD = {
    "model": "local/model",
...
def _seed_run(account, run_id: str):
    """Create the thread, its user turn and one durable generation run inside `account`."""
...
def seed_chat_generation_run(account) -> dict[str, str]:
    """A queued durable run, the state the cancel route acts on."""
...
def seed_chat_generation_run_events(account) -> dict[str, str]:
    """A settled run with a replayable event log, so the SSE route drains and closes."""
...
def seed_chat_attachment(account) -> dict[str, str]:
    """A message carrying one stored upload; the upsert writes the inventory row too."""
...
def seed_chat_project_for_delete(account) -> dict[str, str]:
    """A project with its workspace folders, created by the product's own upsert."""
    from storage import studio_db
    from utils.account_context import run_as
...
def seed_chat_fork_source(account) -> dict[str, str]:
    """A thread with one user turn, the branch point the fork route copies."""
    from storage import studio_db
    from utils.account_context import run_as
...
def seed_chat_fork_tree(account) -> dict[str, str]:
    """The same thread plus a real fork of it, so the count routes report a non-zero count."""
    from storage import studio_db
    from utils.account_context import run_as
